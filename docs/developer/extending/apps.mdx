...
```
:::note
During an app installation, Saleor will try to send an authentication token to provided `tokenTargetUrl`. 
...
The response of the mutation contains the `appJob` which contains all information about installation process.
...
<Chart
  ...
  sequenceDiagram
    ...
    Saleor ->>+ Celery: Schedule installation
    Celery -->>- Saleor: Scheduled
    Saleor -->>- Client: return AppJob
`}
/>
...
```
The query returns simillar response:
```json
{
  "data": {
    "appsInstallations": [
...
This mutation is only available to staff users with the `MANAGE_APPS` permission.
The `appDropFailedInstallation` mutation will delete failed installation.
...
Staff user can't appDeactivate app which has more permissions than he has for his own. 

:::note
The deactivating app will disable all auth-tokens generated for an app. 
